...
def explode_train_target(train_target, target_label):
    ...
    if target_label == "delta_in_2_days":
        ...
        train_sequences = pd.DataFrame(
            train_target[["ICUSTAY_ID", "NOTE_ID", "NOTE_ID_SEQ"]]
            .explode("NOTE_ID_SEQ")
            .drop_duplicates(subset="NOTE_ID_SEQ")
        )
        ...
        train_sequences["label"] = train_sequences["NOTE_ID"].apply(
            lambda x: x in positive_notes
        )
        ...
        train_sequences = pd.DataFrame(
            train_target[["ICUSTAY_ID", "NOTE_ID", "NOTE_ID_SEQ", "label"]]
            .explode("NOTE_ID_SEQ")
            .drop_duplicates(subset="NOTE_ID_SEQ")
        )
        train_sequences.rename(columns={"NOTE_ID": "NOTE_ID_RECENT"}, inplace=True)
...
def merge_embs_to_seq(train_target, train_embs) -> List:
    ...
    for note_id_seq in tqdm(train_target["NOTE_ID_SEQ"]):
        ...
        train_embs_seq = pd.merge(
            note_id_seq_df, train_embs, how="left", on=["NOTE_ID"]
        )
...
def group_train_val(ID) -> Tuple[List[int], List[int]]:
    ...
    num_train_ids = int(0.80 * len(ID_unique_srtd))
...
class Sequential_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        emb_seq_out = torch.zeros(
            size=(self.max_num_notes, self.doc_emb_size), dtype=torch.float
        )
...
class Sequential_Dataset_FAST(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        exploded_idx = pd.merge(
            pd.DataFrame({"NOTE_ID": note_id_seq}, columns=["NOTE_ID"]),
...
if __name__ == "__main__":
    ...
    predictions_example = torch.tensor(
        [
            [0.8, 1, 0.05, 1, 0.02, 0.01],
