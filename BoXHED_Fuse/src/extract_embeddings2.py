...
def generate_dataloader(emb_seq, batch_size):
    params = {
        ...
        "num_workers": 0,
        "drop_last": True,
    }  # do not shuffle here. We will need to merge with IDs later
...
def extract_embeddings(dataloader, model):
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(dataloader):
            ...
            print(
                f"Step {step}/{len(dataloader)} | Time {time() - start_time : .2f} seconds"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--embs-dir", dest="embs_dir", help="dir of train.pt containing embeddings"
    )
    ...
    train_target_path = f'{os.getenv("BHF_ROOT")}/data/targets/till_end_mimic_iv_extra_features_train_NOTE_TARGET_2_{args.note_type[:3]}_all.csv'
    test_target_path = f'{os.getenv("BHF_ROOT")}/data/targets/till_end_mimic_iv_extra_features_test_NOTE_TARGET_2_{args.note_type[:3]}_all.csv'
    train_NOTE_path = f'{os.getenv("BHF_ROOT")}/data/till_end_mimic_iv_extra_features_train_NOTE_{args.note_type[:3]}_all.csv'
    test_NOTE_path = f'{os.getenv("BHF_ROOT")}/data/till_end_mimic_iv_extra_features_test_NOTE_{args.note_type[:3]}_all.csv'
    epoch = re.findall(r"\d+", args.ckpt_model_name)[-1]
    outfolder = f"{MODEL_NAME}_{args.note_type[:3]}_all_out/from_epoch{epoch}"
    out_dir = f'{os.getenv("BHF_ROOT")}/data/final{"/testing" if args.test else ""}/{outfolder}'
    out_embs_dir = f'{os.getenv("BHF_ROOT")}/data/embs{"/testing" if args.test else ""}/{outfolder}'
    ...
    train_target = pd.read_csv(
        train_target_path, converters={"NOTE_ID_SEQ": convert_to_list}
    )
    ...
    if args.test:
        train_target_path = os.path.join(
            os.path.dirname(train_target_path),
            "testing",
            os.path.basename(train_target_path),
        )
        test_target_path = os.path.join(
            os.path.dirname(test_target_path),
            "testing",
            os.path.basename(test_target_path),
        )
        train_NOTE_path = os.path.join(
            os.path.dirname(train_NOTE_path),
            "testing",
            os.path.basename(train_NOTE_path),
        )
        test_NOTE_path = os.path.join(
            os.path.dirname(test_NOTE_path), "testing", os.path.basename(test_NOTE_path)
        )
