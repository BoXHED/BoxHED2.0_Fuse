...
def _generate_target(stay_df, target): # 2 day window
    if target == -1:
        ...
        if delta_times.empty:
            ...
            stay_df.loc[stay_df['t_start'] <= delta_times, target_label] = delta_times - stay_df['t_start']
            # make sure all data after delta_time uses end_time instead
            stay_df.loc[stay_df['t_start'] > delta_times, target_label] = end_time - stay_df['t_start']
        return stay_df
...
OUTPATH_FT_TRAIN = f'/home/ugrads/a/aa_ron_su/BoXHED_Fuse/BoXHED_Fuse/JSS_SUBMISSION_NEW/data/targets/till_end_mimic_iv_extra_features_train_NOTE_TARGET_{args.target}_{args.note_type[:3]}_{args.noteid_mode}.csv'
OUTPATH_FT_TEST = f'/home/ugrads/a/aa_ron_su/BoXHED_Fuse/BoXHED_Fuse/JSS_SUBMISSION_NEW/data/targets/till_end_mimic_iv_extra_features_test_NOTE_TARGET_{args.target}_{args.note_type[:3]}_{args.noteid_mode}.csv'
...
if isinstance(args.target, list):
    for t in target_label:
        train[t] = 0
        test[t] = 0
    train = train[['ICUSTAY_ID','NOTE_ID', 'delta', 't_start','t_start_DT'] + target_label].groupby('ICUSTAY_ID').progress_apply(partial(_generate_target, target=args.target, ))
    test = test[['ICUSTAY_ID','NOTE_ID', 'delta', 't_start', 't_start_DT'] + target_label].groupby('ICUSTAY_ID').progress_apply(partial(_generate_target, target=args.target, ))
else:
    train[target_label] = 0
    test[target_label] = 0
    train = train.groupby('ICUSTAY_ID').progress_apply(partial(_generate_target, target=args.target,))
    test = test.groupby('ICUSTAY_ID').progress_apply(partial(_generate_target, target=args.target,))
